...
/// Handles "git_ship" actions — LLM call via NativeLLMBackend,
/// parse shipping plan, git operations, create PR (non-draft).
///
/// Error policy: fail on any git/github error (matches pyre_client).
/// Mirrors PyreClient.Actions.GitShip — same payload, same result shape.
struct GitShipActionHandler: NativeActionHandler {
    ...
    func execute(
        ...
        do {
            try await GitHelper.checkoutBranch(plan.branchName, workingDir: workingDir)
        ...
        // GitHub: create live PR (non-draft)
        ...
        do {
            let _ = try await GitHubHelper.createPullRequest(config: config, plan: plan, draft: false)
            ...
            await MainActor.run {
                channel.pushAsync("action_complete", [
                    ...
                    "result": [
                        "text": text,
                        "shipping_summary": "Branch: \(plan.branchName), PR: \(plan.prTitle)"
                    ] as [String: Any]
                ])
